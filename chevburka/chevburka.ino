...
const char* ssid = "Keenetic-8700";
const char* password = "U4XY6zvn";
...
IPAddress staticIP(192, 168, 0, 155); // важно правильно указать третье число - подсеть, смотри пояснения выше
IPAddress gateway(192, 168, 0, 1);    // и тут изменить тройку на свою подсеть
IPAddress subnet(255, 255, 255, 0);
IPAddress dns1(77, 88, 8, 8);       // изменить тройку на свою подсеть
...
void build() {
  ...
  GP.UPDATE("nowDate,nowTime,start1Time,stop1Time,rele1Indikator,start21Time,stop2Time,rele2Indikator,start2Time,stop2Time,rele3Indikator,start4Time,stop4Time4,rele4Indikator,tmp,hum,pres,topMark,lovMark,waterWell");
  GP.TITLE("ЧЕМБУРКА", "t2");
  ...
  GP_MAKE_BLOCK_THIN_TAB(
    ...
    GP.LABEL("Темп-ра  : "); GP.NUMBER_F("tmp", "tmp", 23,23);
    GP.BREAK();
    GP.LABEL("Влажность: ");GP.NUMBER_F("hum", "hum", 12,34);
    GP.BREAK();
    GP.LABEL("Давление : ");GP.NUMBER_F("pres", "pres", 712,0);
  );
  //   M_BLOCK_TAB(
  //   "Block Tab",
  //   GP.LABEL("Inputs");
  ...
  GP.NAV_TABS("ЭЛ/ЭНЕР, ВОДА, ОТОПЛЕНИЕ, ВЕНТ");
  
  GP.NAV_BLOCK_BEGIN();
 
  GP_MAKE_BOX(
             GP.LABEL(" А "); GP.NUMBER_F("tmp", "tmp", 220);
             GP.BREAK();
             GP.LABEL(" В ");GP.NUMBER_F("hum", "hum", 230);
             GP.BREAK();
             GP.LABEL(" С ");GP.NUMBER_F("pres", "pres", 230);
   );
  GP.NAV_BLOCK_END();

  GP.NAV_BLOCK_BEGIN();
 
             GP.LABEL("Накопительный бак");
             GP.LABEL("Давление:");
             GP.BREAK();
             GP.LABEL("Верхний уровень:");
             GP.LED_RED("rele1Indikator", setting.rele_1_isOn); GP.BREAK();
             
             GP.LABEL("Нижний уровень:");
             GP.LED_RED("rele1Indikator", setting.rele_1_isOn); GP.BREAK();
    GP_MAKE_BOX(         
             GP.LABEL(" верх уровень "); GP.NUMBER_F("tmp", "tmp", 0);
             GP.BREAK();
             GP.LABEL(" нижний уровень ");GP.NUMBER_F("hum", "hum", 0);
             GP.BREAK();
             GP.LABEL(" скважина ");GP.NUMBER_F("pres", "pres", 220);
   );
  GP.NAV_BLOCK_END();
...
GP.NAV_BLOCK_BEGIN();
 ...
 GP_MAKE_BOX(
             GP.LABEL(" СО2 "); GP.NUMBER_F("tmp", "tmp", 400);
             GP.BREAK();
            //  GP.LABEL(" В ");GP.NUMBER_F("hum", "hum", 230);
            //  GP.BREAK();
            //  GP.LABEL(" С ");GP.NUMBER_F("pres", "pres", 230);
...
void action() {
  ...
  if (ui.update()) {
    ...
    ui.updateFloat("tmp",setting.tmp);
    ui.updateFloat("hum",setting.hum);
    ui.updateFloat("pres",setting.pres);
  }//ui.update()
  if (ui.click()) {
...
void setup() {
  ...
  ntp.setPeriod(3600); // обновлять раз в 600 сек
